...
import { useTypesStore, type ServiceType } from '@/stores/types'
...
function openAdd() {
  ...
  if (store.serviceTypes && store.serviceTypes.length > 0) {
    maxSort = store.serviceTypes.reduce((max, t) => Math.max(max, t.sort_order ?? 0), 0)
  }
...
function exportToCSV() {
  const headers = ['服务名称', '加价(元)', '排序', '状态']
  const rows = store.serviceTypes.map(t => [
    t.name,
    t.price.toString(),
  ...
  link.href = url
  link.download = `服务类型_${new Date().toISOString().split('T')[0]}.csv`
  link.click()
...
async function handleSave() {
  ...
  try {
    if (editingId.value) {
      await store.updateServiceType(editingId.value, typeData)
      ElMessage.success('修改成功')
    } else {
      await store.createServiceType(typeData)
...
async function handleDelete(id: number) {
  await ElMessageBox.confirm('确认删除此服务类型？', '确认删除')
  await store.deleteServiceType(id)
  ElMessage.success('已删除')
}

async function toggleActive(type: ServiceType) {
  try {
    await store.updateServiceType(type.id, { is_active: type.is_active ? 0 : 1 })
...
<template>
  <div>
    ...
    <el-table :data="store.serviceTypes" stripe style="width: 100%; background: var(--card-bg);" empty-text="暂无类型">
      <el-table-column prop="name" label="服务名称" width="150" />
      <el-table-column prop="price" label="加价(元)" width="120">
    ...
    <el-dialog v-model="showDialog" :title="editingId ? '编辑服务类型' : '添加服务类型'" width="420px">
      <el-form label-width="80px">
        <el-form-item label="名称">
          <el-input v-model="form.name" placeholder="如：干洗、熨烫、补色" />
        </el-form-item>
